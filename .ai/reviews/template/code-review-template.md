# LogViewer Code Review {{版本号}} - {{功能标题}}

> **关联 Plan：** [plan-{{版本号}}.md](../plans/plan-{{版本号}}.md)  
> **关联 Review：** [review-{{版本号}}.md](../reviews/review-{{版本号}}.md)  
> **规范依据：** `rtk`、`dotnet-guidelines`、`donet-naming`、`dotnet-winforms-guidelines`  
...
| 领域 | 当前决策 | 用途 / 说明 |
|---|---|---|
| WinForms 窗体基类 | `Form` | 本项目无主题基类，窗体直接继承 Form。 |
| 自绘控件 | `UI/` + OnPaint 离屏绘制 | JsonTreeView 等，Designer-safe。 |
| 配置入口 | `Properties.Settings.Default` | 用户设置持久化。 |
...
| 步骤 | 命令 | 预期 |
|---|---|---|
| Step 0 | `rtk dotnet build .\LogViewer\LogViewer.csproj` | 项目编译通过，无新增错误。 |
| Step 1 | `rtk dotnet format .\LogViewer\LogViewer.csproj --verify-no-changes` | 格式化无差异，说明未引入新的风格漂移。 |
| Step 2 | `rtk dotnet test .\LogViewer\LogViewer.csproj` | 如仓库当前存在可运行测试，则应通过；若无测试项目，需在 review 里注明。 |
...
| 5.9 | `Program.cs` / `MainForm.cs` / `LogServer.cs` 等入口文件命名正确 | ✅ / ❌ / N/A | |
| 5.10 | `LogEntry`、`DeviceConnection`、`JsonTreeView` 等组件命名保持项目风格 | ✅ / ❌ / N/A | |
...
| 6.7 | `android.log.client.settings.json` 是配置入口，不能把运行时配置散落到各模块里 | ✅ / ❌ / N/A | |
...
| # | 检查项 | 状态 | 问题文件（若有） |
|---|---|---|---|
| 7.1.1 | `UI/` 中的自绘控件是否正确实现 `OnPaint` 离屏绘制 + `Dispose` 清理 | ✅ / ❌ / N/A | |
| 7.1.2 | `Network/` 是否零 UI 依赖，不混入界面逻辑 | ✅ / ❌ / N/A | |
| 7.1.3 | 自绘控件颜色是否使用 `SystemColors` 或项目语义常量，无硬编码 | ✅ / ❌ / N/A | |
| 7.1.4 | 新增控件是否优先放入 `UI/`，而不是散落到项目根目录 | ✅ / ❌ / N/A | |
| 7.1.5 | 新增数据模型是否优先放入 `Models/` | ✅ / ❌ / N/A | |
...
| # | 检查项 | 状态 | 问题文件（若有） |
|---|---|---|---|
| 8.1 | 窗体直接继承 `Form`（本项目无主题基类） | ✅ / ❌ / N/A | |
| 8.2 | 主内容宿主是否优先使用 `TableLayoutPanel` 或 `SplitContainer` | ✅ / ❌ / N/A | |
| 8.3 | 列表页是否使用自绘控件或 VirtualMode ListView | ✅ / ❌ / N/A | |
...
| # | 检查项 | 状态 | 问题文件（若有） |
|---|---|---|---|
| 9.2.1 | `android.log.client.settings.json` 是否作为统一配置入口 | ✅ / ❌ / N/A | |
...
| 问题 | 根因 |
|---|---|
| 窗体未按项目规范组织 | 手写逻辑混入 Designer.cs 或文件未放 `UI/` 目录。 |
| 自绘控件未实现 OnPaint 离屏绘制 | 未按 JsonTreeView 模式实现自绘。 |
